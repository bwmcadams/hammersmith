...
import codes.bytes.hammersmith.bson.{BSONDocumentType, ImmutableBSONDocumentParser, SerializableBSONObject, BSONParser}
import codes.bytes.hammersmith.collection.immutable.{Document => ImmutableDocument}
import codes.bytes.hammersmith.collection.Implicits.SerializableImmutableDocument
import codes.bytes.hammersmith.io.MongoFrameHandler
