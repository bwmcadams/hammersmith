...
class BSONTest extends Specification {
  ...
  def hasOID = parsedBSON.get("_id") must haveClass[ObjectID] and beEqualTo(ObjectID(oid.toString()))

  def hasNull = parsedBSON.get("null") must beNull

  def hasMax = parsedBSON.get("max") must haveClass[BSONMaxKey]

  def hasMin = parsedBSON.get("min") must haveClass[BSONMinKey]
  ...
  def hasScopedCode_Code = parsedBSON.get("code_scoped").asInstanceOf[BSONCodeWScope].code must be_==(testCodeWScope.getCode())
  
  def hasScopedCode_Scope = parsedBSON.get("code_scoped").asInstanceOf[BSONCodeWScope].scope must havePairs("foo" -> "bar", "x"-> 5.23)
  
  def hasStr = parsedBSON.get("str").asInstanceOf[String] must be_==(testStr)
  
  def hasSubObj = parsedBSON.get("object").asInstanceOf[Document] must havePairs("foo" -> "bar", "x" -> 5.23)
      
  def hasArray = parsedBSON.get("array").asInstanceOf[BSONList] must contain("foo", "bar", "baz", "x", "y", "z")
