...
class DB(val name: String)(implicit val connection: MongoConnection) extends Logging {
  ...
  protected[hammersmith] def hashPassword(username: String, password: String) = {
  ...
  protected[hammersmith] def hexMD5(str: String): String = hexMD5(str.getBytes)
  protected[hammersmith] def hexMD5(bytes: Array[Byte]) = {
    md5.reset()
    md5.update(bytes)
   ...
   * here.
   */
  protected[hammersmith] var _writeConcern: Option[WriteConcern] = None
