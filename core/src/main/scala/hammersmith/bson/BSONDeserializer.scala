 ...
 */
abstract class BSONDeserializer extends BasicBSONDecoder with Logging {
  ...
  def decodeData(first: Boolean = true): Int = {
  ...
  protected val _getHandle: PartialFunction[Byte, Function1[String, Unit]] = {
    ...
    case CODE_W_SCOPE ⇒ {
      ...
      _in.readInt
      _callback.gotCodeWScope(_: String, _in.readUTF8String, readBasicObject())
    }
    case ARRAY ⇒ {
  ...
  def readBasicObject(): AnyRef = {
    _in.readInt()
    val save = _callback
    val _basic = _callback.createBSONCallback()
...
class DefaultBSONDeserializer extends BSONDeserializer {
  ...
  class DefaultBSONCallback extends Callback {
    ...
    def objectDone() = {
      ...
      org.bson.BSON.applyDecodingHooks(obj).asInstanceOf[BSONDocument]
    }
    ...
    def gotCodeWScope(name: String, code: String, scope: AnyRef) =
      put(name, new org.bson.types.CodeWScope(code, scope.asInstanceOf[BSONObject]))
    ...
    def gotBinary(name: String, t: Byte, data: Array[Byte]) = put(name, new org.bson.types.Binary(t, data))

    def gotBinaryArray(name: String, b: Array[Byte]) = put(name, b)

    def gotDBRef(name: String, ns: String, id: org.bson.types.ObjectId) = put(name, Document("$ns" -> ns, "$id" -> id))

    def gotObjectId(name: String, id: org.bson.types.ObjectId) = put(name, id)

    def gotTimestamp(name: String, time: Int, inc: Int) = put(name, new org.bson.types.BSONTimestamp(time, inc))

    def gotRegex(name: String, pattern: String, flags: String) =
      put(name, new FlaggableRegex(pattern, org.bson.BSON.regexFlags(flags)))

    def gotSymbol(name: String, v: String) = put(name, new org.bson.types.Symbol(v))
    ...
    def put(k: String, v: Any) = cur.put(k, org.bson.BSON.applyDecodingHooks(v))
